package dev.maxc.com2031.lab6_knapsack2;
...
import dev.maxc.com2031.lab4_intervals.IntervalScheduling;
...
public class KnapSack {
    ...
    public int pack() {
        M = new int[items.length + 1][maxWeight + 1];

        Arrays.sort(items, Comparator.comparingInt(o -> o.weight));

        // TODO  complete the array M
        // TODO return the maximum value that can be packed into the knapsack
        ...
        for (int i = 1; i < items.length + 1; i++) {
            for (int w = 0; w < maxWeight + 1; w++) {
                if (items[i - 1].weight > w) {
                    ...
                    M[i][w] = Math.max(M[i - 1][w], items[i - 1].value + M[i - 1][w - items[i-1].weight]);
                }
            }
        }
     ...
     * values.
     *
     * @param numItems    for this many first items from items.
     * @param weightLimit with this remaining weight available
     */
    ...
    public static void testKnapsack(String name, Item[] items, int maxWeight, int expected) {
        ...
        if (maxValue != expected) {
            ...
            System.out.print(knapSack.toString());
